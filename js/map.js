/* 3D location map. Basemap from OpenFreeMap (positron); the Zarea tower and the institutions around it come from
   ...
   Neighbouring buildings are extruded from the vector tiles and stay interactive. Ported from app-proprietati-publice. */
(function () {
  'use strict';
  ...
  function init(P) {
    ...
    const map = new maplibregl.Map({ container: el, style: STYLE_URL, center: [28.8345, 47.0283], zoom: 15.7, pitch: 55, bearing: 150, attributionControl: { compact: true }, cooperativeGestures: true });
    ...
    const data = fetch('data/landmarks.geojson').then((r) => r.json());
    ...
    map.on('load', () => {
      ...
      data.then((fc) => {
        const feats = fc.features.map((f) => Object.assign({}, f, { geometry: offsetFeature(f, 0.4).geometry, properties: Object.assign({}, f.properties, { color: f.properties.cat === 'asset' ? ASSET : CATS[f.properties.cat][1] }) }));
        ...
        if (legend) legend.innerHTML = '<span><i style="background:' + ASSET + '"></i> <b>' + h(P.name) + '</b></span>' + Object.keys(CATS).filter((k) => pois.some((f) => f.properties.cat === k)).map((k) => '<span><i style="background:' + CATS[k][1] + '"></i> ' + h(CATS[k][0]) + '</span>').join('') + '<span class="lmap__hint-src">outlines from OpenStreetMap</span>';
      }).catch(() => { if (legend) legend.innerHTML = '<span><i style="background:' + ASSET + '"></i> <b>' + h(P.name) + '</b></span>'; });
      ...
      const assetInfo = (lngLat) => popup('<div class="bpop"><em>The property for sale</em><b>' + h(P.name) + '</b><span>' + h(P.address) + ' · ' + P.levels + ' floors · ' + P.rooms + ' rooms</span></div>', lngLat);
      ...
      map.on('click', 'z-3d-pois', (e) => { e.originalEvent._zHandled = true; const p = e.features[0].properties; popup('<div class="bpop"><em>' + h(CATS[p.cat][0]) + '</em><b>' + h(p.name) + '</b><span>' + dist(p.dist) + ' from the hotel in a straight line</span><small>' + h(p.ro || '') + ' · outline from OpenStreetMap</small></div>', e.lngLat); });
      if (bl) {
        ...
        map.on('click', 'z-3d', (e) => { if (e.originalEvent._zHandled) return; e.originalEvent._zHandled = true; const f = e.features[0]; const hgt = f.properties && f.properties.render_height; const lv = hgt ? Math.max(1, Math.round(hgt / 3.1)) : null; popup('<div class="bpop"><b>Neighbouring building</b><span>' + (hgt ? 'about ' + Math.round(hgt) + ' m · ' + lv + (lv === 1 ? ' floor' : ' floors') : 'height unknown') + '</span><small>Outline and height from OpenStreetMap. Not part of the offer.</small></div>', e.lngLat); });
      }
